...
export default function Join() {
  ...
  async function onPlayOnce(song: SongLike, turnId?: string) {
    ...
    try {
        ...
        setTimeout(async () => {
          try {
            ...
            if (s && s.device?.is_active && s.is_playing === false) {
              console.log("[PLAY] resume safety", { state: s });
            ...
            console.warn("[PLAY] resume check failed", err);
          }
        }, 700);
        return;
      }
      ...
      setLastPlayTrackStatus(`fallback ${playResp.status}`);
      if (playResp.ok) {
        hasPlayedThisTurnRef.current = true;
        setHasPlayedThisTurn(true);
    ...
    return (
      <div className="min-h-screen bg-zinc-900 p-6 text-white">
        ...
        <div className="mt-4 flex gap-2">
          ...
          <button onClick={() => guess("before")} className="rounded bg-blue-600 px-3 py-2">Before</button>
          <button onClick={() => guess("after")} className="rounded bg-purple-600 px-3 py-2">After</button>
          ...
          {myTurn ? (
            <div>
              {!currentSong ? (
                ...
                <div>
                  <div className="text-xs opacity-70">Playing via Spotify: {currentSong?.name} EUR" {currentSong?.artists}</div>
